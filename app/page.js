...
function ProductCard({ product, settings }) {
  ...
  return (
    <motion.div whileHover={{ y: -4 }} transition={{ duration: 0.2 }}>
      <Card className="glass border-white/8 overflow-hidden group h-full flex flex-col">
        <Link href={`/products/${product.slug}`} className="block relative aspect-square overflow-hidden bg-zinc-950">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {product.tag && <Badge className="bg-white text-black border-0 font-medium text-[10px]">{product.tag}</Badge>}
            {discount > 0 && <Badge className="bg-black/80 text-white border-white/10 font-medium text-[10px]">-{discount}%</Badge>}
          </div>
          ...
          {items.map((it, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} viewport={{ once: true }}>
              <Card className="glass border-white/8 p-6 h-full">
                <div className="h-11 w-11 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center mb-4">
        ...
        <div className="grid md:grid-cols-4 gap-8">
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="h-9 w-9 rounded-lg bg-white/10 border border-white/10 flex items-center justify-center"><Wind className="h-5 w-5 text-white" /></div>
...
function WhatsAppFloat({ settings }) {
  return (
    <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1 }} onClick={() => { trackWhatsAppClick({ source: 'float' }); openWhatsApp({ number: settings?.whatsappNumber, message: settings?.whatsappMessage }) }} className="fixed bottom-6 right-6 z-40 h-14 w-14 rounded-full bg-white text-black shadow-lg shadow-black/40 flex items-center justify-center hover:bg-white/90">
